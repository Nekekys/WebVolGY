...
if ((isset($_GET["name"])) || (isset($_GET["brand"])) || (isset($_GET["ot"])) || (isset($_GET["do"]))) {
    ...
    if ($_GET["brand"] != "0") {
        if ($check) {
            $query .= " AND goods.id_brand = '" . mysqli_real_escape_string($mysql,$_GET["brand"]) . "'";
        } else {
            $query .= " WHERE goods.id_brand = '" . mysqli_real_escape_string($mysql,$_GET["brand"]) . "'";
        }
        $check = true;
...
if($_SESSION['user']){
    while ($row = $result->fetch_assoc()) {
        $i++;
        echo "<tr><th scope='row'><img src='logics/get_image.php?img=" . $row["img_path"] . "' style='width: 80px' alt='Авторизуйтесь'></th>
                                <td>" . $row['name'] . "</td>
