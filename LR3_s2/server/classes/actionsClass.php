<?php 
    require_once($_SERVER['DOCUMENT_ROOT'] . '/server/classes/databaseClass.php');

    class ActionBrand {

        public static function addAction(string $name_) : string{
            $mysql = Database::connection();
            $name = mysqli_real_escape_string($mysql,$name_);
            $query = "INSERT INTO brands (`id_brand`, `name_brand`) VALUES ( NULL, '$name')";
            $result = $mysql->query($query);
            $returnId = $mysql->insert_id;
            return $returnId;
        }
    ...
    class ActionGoods {
        public static function addAction(string $name_, int $cost_, string $des_, int $brand_, string $fileName_) : string{
            $mysql = Database::connection();
            ...
            $query = "INSERT INTO goods (`id`, `name`, `description`, `cost`, `id_brand`, `img_path`) 
                        VALUES ( NULL, '$name', '$des', '$cost', '$brand', '$fileName_')";
            $result = $mysql->query($query);
            $returnId = $mysql->insert_id;
            return $returnId;
        }
        ...
        public static function editActionWhithFile(int $id_,string $name_, int $cost_, string $des_, int $brand_, string $fileName_) : bool{
            $mysql = Database::connection();
            $id = mysqli_real_escape_string($mysql,$id_);
            $name = mysqli_real_escape_string($mysql,$name_);
            $cost = mysqli_real_escape_string($mysql,$cost_);
            ...
            $query = "SELECT id, img_path  FROM goods WHERE id = $id";
            $result = $mysql->query($query);
            $arr = [];
            while ($row = $result->fetch_assoc()) {
                $arr[] = $row;
            }
            unlink("../files/".$arr[0]['img_path']);

            $query = "UPDATE goods SET  name = '$name', description = '$des', cost = '$cost', id_brand = '$brand', img_path = '$fileName_' WHERE id = $id";
            $result = $mysql->query($query);
            return $result;
        }
